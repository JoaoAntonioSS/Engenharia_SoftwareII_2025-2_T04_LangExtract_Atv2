...
MODEL_NAME = "sentence-transformers/all-mpnet-base-v2" # Modelo Mais Preciso
QTD_COMMITS = 100
...
padrao_bom = [
    "feat: add functionality", "fix: resolve bug", "docs: update readme",
    "Add new feature support", "Update dependency versions", "Remove unused code",
...
print("\n--- REANALISANDO COM MODELO DE ALTA PRECISÃO ---")
...
print("\n" + "="*50)
print(f"RELATÓRIO DE GOVERNANÇA (MPNET-BASE)")
print("="*50)
...
plt.title(f'Análise de Governança (MPNet)\nProjeto: {REPO_NAME}')
plt.show()
...
REPO_OWNER = "google"
REPO_NAME = "langextract"
MODEL_NAME = "sentence-transformers/paraphrase-MiniLM-L3-v2" # Modelo Mais Rápido/Leve
QTD_COMMITS = 100
...
print("\n--- REANALISANDO COM MODELO DE ALTA VELOCIDADE ---")
...
print("\n" + "="*50)
print(f"RELATÓRIO DE GOVERNANÇA (PARAPHRASE-L3)")
print("="*50)
print(f"Commits Estruturados (Conventional + Imperative): {good_count}")
print(f"Commits Genéricos / Sem Padrão Claro: {bad_count}")
print(f"Nível de Maturidade Real: {maturity:.1f}%")
...
plt.figure(figsize=(7, 7))
labels = [f'Estruturado ({maturity:.1f}%)', f'Ad-hoc ({100-maturity:.1f}%)']
colors = ['#8E24AA', '#FFAB91']
plt.pie([good_count, bad_count], labels=labels, colors=colors, autopct='%1.1f%%', startangle=140, shadow=True)
plt.title(f'Análise de Governança (Paraphrase L3)\nProjeto: {REPO_NAME}')
plt.show()
